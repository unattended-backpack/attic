...
RUN cargo build --release -p attic-server
...
COPY --from=builder /build/target/release/atticd /usr/local/bin/atticd
COPY --from=builder /build/target/release/atticadm /usr/local/bin/atticadm
